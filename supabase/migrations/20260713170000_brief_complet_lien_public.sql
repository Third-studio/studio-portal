-- 2026-07-13 (bis) — Le formulaire public ?nouveau=TOKEN porte désormais le brief
-- complet (mêmes éléments que le brief client normal : objectif, cible, durée, ton,
-- budget, livrables, éléments de marque, dates, références, notes, services, musique).

-- 1. get_project_invite renvoie aussi les services additionnels actifs
--    (le formulaire public ne peut pas lire service_types via l'anon key)
...
-- 2. Nouvelle signature : contact + brief complet en jsonb (clés whitelistées côté SQL)
drop function if exists public.create_project_from_invite(text, text, text, text, text, text, text);
